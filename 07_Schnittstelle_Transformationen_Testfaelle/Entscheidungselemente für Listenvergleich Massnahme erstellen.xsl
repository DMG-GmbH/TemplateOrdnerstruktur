...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:variable name="ID" as="xs:integer">50003302631000</xsl:variable>
    <xsl:template match="/">
    ...
    <SequenzielleListe Id="500003302631000">
    <Comment>Maßnahmen Vergleiche</Comment>
    <Active>true</Active>
        <xsl:for-each select="//Metadatenfeld">
        <xsl:variable name="pos" select="position()"/>
        <xsl:variable name="Verweis" select="." />
        <xsl:variable name="IDinner" as="xs:integer">10003302631000</xsl:variable>
        <xsl:variable name="IDSeq" as="xs:integer">100003302631000</xsl:variable>
        <xsl:variable name="IDSeqVar" as="xs:integer">200003302631000</xsl:variable>
        <xsl:variable name="IDSeqWF" as="xs:integer">300003302631000</xsl:variable>
        <xsl:variable name="IDinnerWF" as="xs:integer">20003302631000</xsl:variable>
        <xsl:variable name="IDinnerBefehl" as="xs:integer">30003302631000</xsl:variable>
        ...
        <xsl:if test="@Datentyp != 'ANLAGEN'">
            <Entscheidungselement>
                ...
                <Entscheidungszweig>
                    ...
                    <Comment>ListenelementSetzen(V.Z.Logbuch.ListeMassnahme, "Spalte", castToString(<xsl:value-of select="$Titel"/>), ZaehlerLogbuch) </Comment>
                        <Active>true</Active>
                        <Befehl>
                            <Ausdruck>ListenelementSetzen(V.Z.Logbuch.ListeMassnahme, "Spalte", castToString(<xsl:value-of select="$Titel"/>), ZaehlerLogbuch) </Ausdruck>
                        </Befehl>
                    </BefehlAusfuehren>
                    <BefehlAusfuehren>
                        <xsl:attribute name="Id">
                    ...
                    <BefehlAusfuehren>
                        <xsl:attribute name="Id">
                            <xsl:value-of select="$IDinnerBefehl + $pos + 900" />
                        </xsl:attribute>
                        <Comment>ListenelementSetzen(V.Z.Logbuch.ListeMassnahme, "Massnahme", Listenelement(V.Z.<xsl:value-of select="$DMName"/>.<xsl:value-of select="$Liste"/>, i, "Bezeichnung"), ZaehlerLogbuch)</Comment>
                        <Active>true</Active>
                        <Befehl>
                            <Ausdruck>ListenelementSetzen(V.Z.Logbuch.ListeMassnahme, "Massnahme", Listenelement(V.Z.<xsl:value-of select="$DMName"/>.<xsl:value-of select="$Liste"/>, i, "Bezeichnung"), ZaehlerLogbuch) </Ausdruck>
                        </Befehl>
                    </BefehlAusfuehren>
